...
writeClauses(MaxCost):-
    ...
    exactlyOneMatchBetweenTwoTeams,
    defineHome,
    defineDouble,
    eachTeamNoTriples,
    noDoublesOnCertainRounds,
    noPlayHome,
    everyRoundAtLeastOneMatchBetweenTwoTvTeams,
    maxCost(MaxCost),
    true,!.
...
eachTeamNoTriples:-
    ...
    round(R2),
    R2 is R1+1,
    writeClause([\+double-T-R1, \+double-T-R2]),
    fail.
eachTeamNoTriples.
...
everyRoundAtLeastOneMatchBetweenTwoTvTeams:-
    round(R), findall(match-S-T-R, tvMatch(S,T), Lits), atLeast(1,Lits), fail.
everyRoundAtLeastOneMatchBetweenTwoTvTeams.
